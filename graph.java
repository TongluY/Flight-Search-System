...
public class Graph<NodeType, EdgeType extends Number> implements GraphADT<NodeType, EdgeType> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            List<NodeType> newseq = new LinkedList<>();
            newseq.addAll(copyPath.dataSequence);
            newseq.add(extendBy.target.data);
            this.dataSequence = newseq;
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) throws NoSuchElementException {
        if (!containsVertex(start)) {
            throw new NoSuchElementException("The starting vertex cannot be found");
        }
        if (!containsVertex(end)) {
            throw new NoSuchElementException("The ending vertex cannot be found");
        }
        // list of visited nodes
        Hashtable<Vertex, Path> visited = new Hashtable<Vertex, Path>();
        Hashtable<Vertex, Boolean> checked = new Hashtable<Vertex, Boolean>();
        for (Vertex v : vertices.values()) {
            visited.put(v, new Path(v));
            checked.put(v, false);
        }
        // set up the priority queue
        // with a path with only the starting point (also saved as the min path)
        Vertex startVertex = vertices.get(start);
        PriorityQueue<Path> pq = new PriorityQueue<Path>();
        pq.add(new Path(startVertex));
        // Path returnPath = new Path(new Vertex(start));
        while (!pq.isEmpty()) {
            // remove the shortest path from pq
            Path minPath = pq.poll();
            // add minimum path to the visited hashtable
            if (checked.get(minPath.end) == false) {
                if (visited.get(minPath.end).distance == 0 || minPath.distance < visited.get(minPath.end).distance) {
                    visited.replace(minPath.end, minPath);
                    checked.replace(minPath.end, true);
                }
            }
            // add new path to the priority queue
            for (int i = 0; i < minPath.end.edgesLeaving.size(); i++) {
                Path pathNew = new Path(minPath, minPath.end.edgesLeaving.get(i));
                if (checked.get(minPath.end.edgesLeaving.get(i).target) == false) {
                    pq.add(pathNew);
                }
            }
        }
        Path pathReturn = visited.get(vertices.get(end));
        if (checked.get(vertices.get(end)) == false) {
